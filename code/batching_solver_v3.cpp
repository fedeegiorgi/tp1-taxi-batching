    ...
    for (int fila = 0; fila < n; fila++){
        ...
        for (index; index < n; index++){
            ...
            if (this->_instance.pax_tot_fare[index] < 0) {
                costo = (matrix[fila][index]*10 + this->_instance.pax_trip_dist[index]*10) / (abs(this->_instance.pax_tot_fare[index])); // Multiplicación por 10 para que los costos sean enteros.
            } else if (this->_instance.pax_tot_fare[index] == 0) {
                costo = (matrix[fila][index]*10 + this->_instance.pax_trip_dist[index]*10) / 1; // Multiplicación por 10 para que los costos sean enteros.
            } else {
                costo = (matrix[fila][index]*10 + this->_instance.pax_trip_dist[index]*10) / this->_instance.pax_tot_fare[index]; // Multiplicación por 10 para que los costos sean enteros.
            }
            // A medida que recorro la matriz, para cada valor calculo el ratio r' especificado en el informe (calculo todos los r'_ij)
            // y voy agregandolos al vector.
